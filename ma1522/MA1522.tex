...
Let $V$ be a subspace of \R{n}. Suppose $S =  \{  u_{1}  ,\dots, u_{k}    \}$ are \textbf{bases} for the subspace $V$. Let \textbf{P} be the transition matrix from $T$ to $S$. Then for any vector $w$ in $V$, \[ [\mathbf{w}]_{S} = \mathbf{P}[\mathbf{w}]_{T}. \] }
...
Suppose $S = \{ u_{1},\dots, u_{k}\}$ and $T = \{ v_{1},\dots, v_{k} \}$ are \textbf{bases} for a subspace $V$ of \R{n}. Let \textbf{P} be the transition matrix from $T$ to $S$. Then $P^{-1}$ is the transition matrix from $S$ to $T$.}
